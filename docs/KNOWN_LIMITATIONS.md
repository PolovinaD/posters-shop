...
**Why it persists**: The reservation TTL is hardcoded at `services/orders/main.py:184` (`ttl_minutes=15`, matching the inventory-side default at `services/inventory/schemas.py:33`) and starts ticking when stock is reserved inside `create_order` — before the customer has been redirected to Stripe at all, and regardless of how long they then spend there. Inventory's expiry worker polls every 30 seconds (`services/inventory/main.py:124`), releases each expired reservation and posts to `POST /internal/orders/{order_id}/reservation-expired` (`services/inventory/main.py:52` → `services/orders/main.py:703`), which flips an order still sitting in `reserved` to `cancelled` and emits `ORDER_CANCELLED`. Nothing along that path knows whether a Stripe checkout session is open: `create_checkout` (`services/orders/main.py:550`) records `checkout_session_id` on the order but does not touch the reservation's `expires_at`, and the expiry worker never consults it. Hosted Checkout is off-platform and takes as long as the customer takes, so the two clocks are simply unrelated and the slower customer loses the race.
