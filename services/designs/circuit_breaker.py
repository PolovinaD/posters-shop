...
Failure classification (per D-03):
  ...
  - Ignores: 4xx business errors (PromptRejected, ProviderConfigError, CatalogRejectedError, InventoryRejectedError)
"""
import asyncio
import os
import time
...
# Exceptions that are business answers, not outages, and must NOT trip the circuit:
#   PromptRejected / ProviderConfigError  - the image provider answered 4xx (refusal or our misconfiguration)
#   CatalogRejectedError / InventoryRejectedError - catalog/inventory answered 4xx at print time (09-04)
_BUSINESS_ERROR_NAMES = frozenset({
    "PromptRejected",
    "ProviderConfigError",
    "CatalogRejectedError",
    "InventoryRejectedError",
})
